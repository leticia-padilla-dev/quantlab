...
- [Stepbit I/O contract v1](./stepbit-io-v1.md)
- [QuantLab / Stepbit boundaries](./quantlab-stepbit-boundaries.md)
...
Stepbit is an external consumer. It may request a bounded action and inspect the resulting artifacts, but it does not own QuantLab lifecycle, execution policy, risk policy, broker submission, or strategy promotion.
...
- no Stepbit-owned promotion policy
- no parsing of private QuantLab internals
- no dependency on Desktop UI state

## Invocation

Stepbit should invoke QuantLab with an explicit project interpreter from the QuantLab repository root.
...
```powershell
$signalFile = "$env:TEMP\quantlab-stepbit\run-signals.jsonl"
$jsonRequest = @{
  schema_version = "1.0"
...
```powershell
$sweepOut = "$env:TEMP\quantlab-stepbit\sweep-outputs"
$signalFile = "$env:TEMP\quantlab-stepbit\sweep-signals.jsonl"
$jsonRequest = @{
  schema_version = "1.0"
...
Stepbit should provide a signal file path and consume it as JSON Lines.
...
```json
{
  ...
  "artifacts_path": "C:\\Users\\marce\\AppData\\Local\\Temp\\quantlab-stepbit\\sweep-outputs\\20260509_202930_grid_631fada",
  "report_path": "C:\\Users\\marce\\AppData\\Local\\Temp\\quantlab-stepbit\\sweep-outputs\\20260509_202930_grid_631fada\\report.json",
  "summary": {
...
Stepbit should not treat stdout text as the result contract. QuantLab currently does not emit a JSON response envelope to stdout.
...
Do not resolve artifact names relative to the Stepbit process working directory.
...
Stepbit should combine exit code with the signal-file and `machine_contract` checks. A zero exit code alone is not enough to prove a usable result.

## Invalid Consumer States

Stepbit must treat these states as invalid or failed, even if QuantLab exits with code `0`:
...
They are not the authority for Stepbit.
...
4. Choose a signal-file path controlled by Stepbit.
...
16. Return machine_contract and resolved artifact paths to Stepbit.
```

## What #61 May Implement

#61 may implement a `QuantLabTool` adapter that:

- launches QuantLab through this CLI contract
- writes a JSON request envelope
- supplies a signal file
- consumes `SESSION_COMPLETED`
- reads `report.json.machine_contract`
- returns canonical result metadata and resolved artifact paths

#61 must not implement:

- broker submit
- live execution
- promotion automation
- strategy policy
- QuantLab-internal imports
- Desktop UI coupling
