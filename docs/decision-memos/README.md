...
Memos should be named using the format:

`YYYY-MM-DD-asset-verdict.md`
...
- `no_trade`: QuantLab evidence is bad/weak, contradictory, or risk is undefined.
...
- `candidate_for_future_supervised_live_review`: Paper evidence is strong enough to consider a future supervised review. **This does not open Stage E and does not authorize live execution.**
...
5. **No Execution Authority**: A memo never authorizes live execution, broker submit, automation, or Stage E.
6. **Revision Discipline**: If a memo is updated after new evidence appears, add a revision note instead of silently changing the original decision context.