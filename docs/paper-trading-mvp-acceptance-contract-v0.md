...
For v0 final certification, use the frozen profile for at least **24 continuous wall-clock hours** and at least **24 completed scheduled decision opportunities**; if the chosen cadence cannot provide 24 opportunities in 24 hours, extend the window until it does. Each opportunity records input receipt and freshness, decision or no-trade, session state, and any resulting action/outcome. A process restart exercised for M03 remains within the same window. Unexplained operational gaps, unreconciled ambiguity or lost evidence prevent completion; two consecutive opportunities missed because of an internal stall or unexplained stale feed are `FAIL`. An explicitly identified external outage may be recorded as an interruption and extend the window, but its missed opportunities do not count as completed. No market-generated trade is required: deterministic fixtures cover entry, exit and no-trade; the real-time window proves continuity, scheduling, persistence and observability.
...
The Adopt/Build spike evaluates Freqtrade first against each applicable M01–M13 scenario using the frozen profile, deterministic fixtures, fault injection, native-capability inspection, restart/idempotency/accounting/authority tests and short runtime smoke where needed. It selects an engine; it does not certify the Paper MVP or require the full real-time observation window. For **engine selection only**, M03, M05, M06, M07 and M11 are critical; M12 is critical when evaluating an external engine, and M13 is critical only when the spike includes venue-connected validation. M01, M02, M04, M08, M09 and M10 remain `MUST` for final Paper MVP acceptance. Do not reinterpret `PAUSE`, restart, accounting, or authority for a preferred engine. No engine is implemented or selected by this document.
...
For each applicable scenario, record engine-native result, QuantLab-owned gap, proposed mitigation, spike evidence and `PASS` / `PARTIAL` / `FAIL`. A non-critical `PARTIAL` may remain with Freqtrade only if QuantLab can close the gap completely in its own layer/adapter, without forking or changing engine core, trusting unverifiable state, weakening a critical invariant, or changing the contract; the closure must be deterministically testable against that same contract. Document such a gap as implementation work, not an MVP pass, and do not require a NautilusTrader comparison solely for it.

Evaluate NautilusTrader against **exactly the same** profile and criteria if any critical scenario is `FAIL` or remains `PARTIAL`, any gap requires modifying/forking engine core, the engine exposes insufficient state to verify an invariant, or the proposed solution reinterprets or weakens the contract. If neither engine can meet the critical contract, define only the demonstrated missing component for a build decision. Then record an ADR, select the engine and make an **ENGINE FREEZE**: stable until failed acceptance or operational evidence invalidates it, not reopened for curiosity or tool preference. Only after that decision and MVP construction, run all applicable `MUST` scenarios end-to-end with definitive evidence and the predeclared real-time paper observation; only then may Paper MVP be declared `PASS`. Do not search indefinitely.
